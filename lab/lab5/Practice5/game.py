...
class Game:
    ...
    def is_terminal(self, state):
        ...
        return not self.actions(state)  # 棋盘中没有可以下的位置了
...
class Board(defaultdict):
    ...
    def new(self, changes: dict, **kwds) -> 'Board':
        ...
        board1 = Board(width=self.width, height=self.height,
                       **kwds)  # new一个新的board1
...
class TicTacToe(Game):
    ...
    def result(self, board, square):
        ...
        board = board.new({square: player}, to_move=(
            'O' if player == 'X' else 'X'))  # 这一步自动切换player
        ...
        if not win:
            ...
            if player == 'X':
                ...
                utility = -1
        '''
        board.utility = (0 if not win else +1 if player ==
...
def k_in_row(board, player, square, k):
    ...
    def in_row(
        x, y, dx, dy): return 0 if board[x, y] != player else 1 + in_row(x + dx, y + dy, dx, dy)
