...
class ProblemSolvingAgent:
    ...
    def Astar(self, obstacles, start_pos, goal_pos):
        ...
        q.put((0, 0, start_pos))  # f=g+h, cur→start, (x, y)
        ...
        while not q.empty():
            ...
            if cur[2] == goal_pos:
                break
            for i in list(self.neighbours_of(obstacles=obstacles, node=cur[2])):
                if i[0] not in visited:
                    q.put(
                        (i[1] + cur[1] + self.euclidean_distance(i[0], goal_pos), i[1] + cur[1], i[0]))
                    visited.append(i[0])
                    parent[i[0]] = cur[2]
