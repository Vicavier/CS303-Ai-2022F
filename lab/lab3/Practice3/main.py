...
# whether to generate gif file. If true, the real time display would be much slower.
save_fig = True
...
# agent algorithm
# The agent implementation, you should complete the agent.py
agent = ProblemSolvingAgent()
while True:
    case = input(
        'Please input the test case number, or input q to quit. ') or '1'
    if case.strip() == 'q':
        break
    # environment world
    ...
    while True:
        ...
        if algorithm.strip() == 'c':
            ...
            path, visited = agent.solve_by_searching(
                obstacles, start_point, goal_point, algorithm)
