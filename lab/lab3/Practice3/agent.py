...
class ProblemSolvingAgent:
    ...
    def DFS(self, obstacles, start_pos, goal_pos):
        # if len(list(self.neighbours_of(obstacles=obstacles, node=start_pos))) == 0:
        #     return [], []
        # path, visited = [], []
        # s = deque()
        # path.append(start_pos)
        # visited.append(start_pos)
        # for i in list(self.neighbours_of(obstacles=obstacles, node=start_pos)):
        ...
        #     path_t, visited_1 = self.DFS(
        #         obstacles=obstacles, start_pos=cur, goal_pos=goal_pos)
        #     if len(path_t) == 0 and not s:
        #         return [], []
        #     for _ in path_t:
        #         path.append(_)
        #     for _ in visited_1:
    ...
    def BFS(self, obstacles, start_pos, goal_pos):
        ...
        while not q.empty():
            ...
            if cur == goal_pos:
                break
            for i in list(self.neighbours_of(obstacles=obstacles, node=cur)):
                if i[0] not in visited:
                    visited.append(i[0])
                    q.put(i[0])
                    parent[i[0]] = cur
