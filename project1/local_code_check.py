...
class ChessCase:
    def __init__(self, chessboard_size):
        ...
        """
        self.chessboard = np.zeros(
            (chessboard_size, chessboard_size), dtype=np.int)
        self.rational_steps = []
    ...
    def __check_result(self, chessboard, result, color=-1):
        try:
            ...
            for i in self.agent.candidate_list:
                if i not in result:
                    print("user choice: {}".format(list(i)))
                    return False
            if result is not None:
                for i in result:
                    if tuple(i) not in self.agent.candidate_list:
                        print("user not choice: {}".format(list(i)))
                        return False
    ...
    def __check_advance_chessboard(self):
        ...
        for case in case_list:
            if not self.__check_result(case.get_board(), case.get_rational_steps(), color=-1):
                ...
                print("Haven't pass the test")
                print(case.get_board())
                print(case.get_rational_steps())
            else:
                pass_num += 1
            size += 1
...
if __name__ == "__main__":
    code_checker = CodeCheck("myAIv0-3-2.py", 8)
