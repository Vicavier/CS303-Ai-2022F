...
class Agent:
    ...
    def get_action(self,
        ...
        while True:
            ...
            ctps_inter =  torch.randn((BATCH, N_CTPS-2, 2)) * torch.tensor([N_CTPS-2, 2.]) + torch.tensor([1., -1.])
            ...
            optimizer = torch.optim.AdamW([ctps_inter], lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
            # optimizer = torch.optim.SGD([ctps_inter], lr=lr)
            for i in range(it):
                optimizer.zero_grad()
                traj = compute_traj(ctps_inter, BATCH)
                score = new_evaluate(traj, target_pos, target_scores, RADIUS)
                score.backward(gradient=torch.FloatTensor([1.0]*BATCH))
                optimizer.step()
                ctps_inter.data = ctps_inter.data + lr * ctps_inter.grad / torch.norm(ctps_inter.grad)
            values = evaluate(compute_traj(ctps_inter, BATCH), target_pos, target_scores, RADIUS)
            index = torch.argmax(values)
            if values[index] > ctps[1]:
                ctps = [ctps_inter[index],values[index]]
        print(time.time() - start)
        return ctps[0]

def compute_traj(ctps_inter: torch.Tensor, BATCH: int):
    ...
    ctps = torch.cat((
        torch.tensor([[[0., 0.]]]*BATCH, device=ctps_inter.device),     #ctps:[[0,0],...,[5,0]]
        ctps_inter,
        torch.tensor([[[N_CTPS, 0.]]]*BATCH, device=ctps_inter.device)
    ),dim=1)
...
def _splev_torch_impl(x: torch.Tensor, t: torch.Tensor, c: torch.Tensor, k: int):
    """
        ...
        c: (n_ctps, dim)
    """
    # assert t.size(0) == c.size(1) + k + 1, f"{len(t)} != {len(c[0])} + {k} + {1}" # m= n + k + 1
