...
class Agent:
    ...
    def get_action(self,
        ...
        while time.time() - start < 0.25:
            ...
            if score[index] > ctps[1]:
                ctps = [ctps_inter[index],score[index]]
        # gradient descent
        it = 10
        lr = 0.4
        ctps_inter =  ctps[0].detach().clone()
        ...
        for i in range(it):
            ...
            score = new_evaluate(traj, self.target_pos, self.target_scores, RADIUS)
            score.backward()
            optimizer.step()
            ctps_inter.data = ctps_inter.data + lr * ctps_inter.grad / torch.norm(ctps_inter.grad)
        value = evaluate(compute_traj(ctps_inter), self.target_pos, self.target_scores, RADIUS)
        if value > ctps[1]:
            ctps = [ctps_inter,value]
